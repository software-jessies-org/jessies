...
public class PTabSegment extends PAbstractSegment {
    ...
    private int tabWidth() {
        return (SPACES_PER_TAB * getFontMetrics().charWidth(' '));
    }
    ...
    private void paintArrow(Graphics2D g, int x, int yMiddle, int yBaseline) {
        g.drawLine(x, yBaseline, x, yBaseline - getFontMetrics().getMaxAscent());
    ...
    public void paint(Graphics2D g, int x, int yBaseline) {
        ...
        final int xStop = x + getDisplayWidth(x) - 1;
        ...
        for (int tabStopX = xStop; tabStopX > (x + MIN_TAB_WIDTH_IN_PIXELS); tabStopX -= tabWidth) {
            paintArrow(g, tabStopX, yMiddle, yBaseline);
        }
    }
}
