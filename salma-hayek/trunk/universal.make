...
INSTALLER_SOURCE_DIRECTORY := $(wildcard native/$(TARGET_OS)/WiX)
INSTALLER_BINARY = .generated/$(INSTALLER_SOURCE_DIRECTORY)/$(TARGET_OS)/$(PROJECT_NAME).msi
NATIVE_TGZ_FILE_OF_THE_DAY = $(PROJECT_NAME)-$(TARGET_OS)-$(DATE).tgz
NATIVE_DIST_FILE_OF_THE_DAY := $(if $(INSTALLER_SOURCE_DIRECTORY),$(INSTALLER_BINARY),../$(NATIVE_TGZ_FILE_OF_THE_DAY))
...
../$(NATIVE_TGZ_FILE_OF_THE_DAY): $(ALL_NATIVE_TARGETS)
	cd .. && \
	tar -zcf $(NATIVE_TGZ_FILE_OF_THE_DAY) $(patsubst $(PROJECT_ROOT)/%,$(PROJECT_NAME)/%,$(ALL_NATIVE_TARGETS))
